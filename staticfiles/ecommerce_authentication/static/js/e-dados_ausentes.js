function enviardados() {
  const form = document.querySelector("#form-checkout"),
    ...
    formBairro = form.querySelector("#form-bairro").value,
    formEndereco = form.querySelector("#form-endereco").value,
...
function validateField(input) {
  ...
  const removeErrorMessage = () => {
    if (
      ...
      input.nextElementSibling.classList.contains("text-helper")
    ) {
  ...
  function addErrorMessage(input, message) {
    ...
    if (
      ...
      previousErrorMessage.classList.contains("text-helper")
    ) {
  ...
  if (input.value.trim() === "") {
    ...
    if (!/^\d*$/.test(input.value)) {
      ...
      addErrorMessage(
        input,
        `O campo ${labelName} deve conter somente números!`
      );
    } else {
      removeErrorMessage();
    }
    ...
    if (input.id === "phone") {
      ...
      input.addEventListener("blur", () => {
        ...
        if (!/^\d{11}$/.test(phoneValue) && input.value.length < 11) {
          addErrorMessage(
            input,
            `O campo ${labelName} deve conter 11 dígitos numéricos!`
          );
        } else {
    ...
    if (input.id === "form-cep") {
      ...
      input.addEventListener("blur", () => {
        ...
        if (!/^\d{8}$/.test(phoneValue) && input.value.length < 8) {
          addErrorMessage(
            input,
            `O campo ${labelName} deve conter 8 dígitos numéricos!`
          );
        } else {
          // Remova a formatação atual (se houver) antes de aplicar a máscara
          input.value = phoneValue;
    ...
    if (input.id === "codearea") {
      ...
      input.addEventListener("blur", () => {
        ...
        if (!/^\d{3}$/.test(phoneValue) && input.value.length < 3) {
          addErrorMessage(
            input,
            `O campo ${labelName} deve conter no mínimo 3 dígitos numéricos!`
          );
        } else {
          // Remova a formatação atual (se houver) antes de aplicar a máscara
          input.value = phoneValue;
      ...
      cpfCnpjInput.addEventListener("input", () => {
        ...
        if (input.value === "cpf") {
          ...
          input.addEventListener("blur", () => {
            ...
            if (
              !/^\d{14}$/.test(phoneValue) &&
              cpfCnpjInput.value.length < 14
            ) {
              addErrorMessage(
                cpfCnpjInput,
                `O campo ${labelName} deve conter 14 dígitos numéricos!`
              );
            } else {
              // Remova a formatação atual (se houver) antes de aplicar a máscara
              cpfCnpjInput.value = phoneValue;
...
inputCep.addEventListener("input", () => {
  ...
  const rua = document.querySelector("#form-endereco");
...
function removeErrorForField(input) {
  const errorMessages = document.querySelectorAll(
    `label[for="${input.id}"] .text-helper`
  );
  if (
    input.classList.contains("error-data") &&
    input.nextElementSibling.classList.contains("text-helper")
  ) {
    input.nextElementSibling.remove();
    input.classList.remove("error-data");
